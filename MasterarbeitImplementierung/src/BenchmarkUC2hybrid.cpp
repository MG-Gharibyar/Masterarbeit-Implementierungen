...
class FHEBenchmark {
    ...
    public:
        FHEBenchmark(ScalingTechnique rescaleTech, SecurityLevel seclvl, int numparties, int scalemodsize, int firstmodsize, COMPRESSION_LEVEL complvl, int depth){
            ...
            parameters.SetSecurityLevel(seclvl);
            if(seclvl == SecurityLevel::HEStd_NotSet){
        ...
        /**
         * @brief Encrypts the raw input data of fleetMap and stores it into struct FleetDataCipher
        */
        void EncryptData(){
            ...
            for (const auto& [truckId, data] : fleetMap) {
                ...
                TIC(t);
                datacipher.cargoWeight = cryptoContext->Encrypt(parties[numParties-1].kpShard.publicKey, ptxt);
            ...
            std::cout << "Size of fleetMap (no. of trucks): " << fleetMap.size() << std::endl;
        ...
        */
        ...
        Ciphertext<DCRTPoly> Bootstrap(Ciphertext<DCRTPoly> ct){
            ...
            auto crp = cryptoContext->IntMPBootRandomElementGen(parties[numParties-1].kpShard.publicKey);
            ...
            for(size_t j = 0; j < parties.size(); j++){
                std::vector<Ciphertext<DCRTPoly>> sharesPair = cryptoContext->IntMPBootDecrypt(parties[j].kpShard.secretKey, c1, crp);
                sharesPairVec.push_back(sharesPair);
            }

            auto aggregatedSharesPair = cryptoContext->IntMPBootAdd(sharesPairVec);
            return cryptoContext->IntMPBootEncrypt(parties[numParties-1].kpShard.publicKey, aggregatedSharesPair, crp, cadj);
        }
        ...
        void CalcCipherPearsonCorr(){
            ...
            for (const auto& [truckId, data] : CipherInput) {
                ...
                PCorResultNum.push_back(numerator);
                ...
                TIC(t_bs);
                denom1 = Bootstrap(denom1);
                denom2 = Bootstrap(denom2);
                bm_results.PCor_BS_Dur = TOC_US(t_bs) / 1000.0;

                auto denom3 = cryptoContext->EvalMult(denom1,denom2);
                
                PCorResultDenom.push_back(denom3);
            ...
            std::cout << "Level after Pearson calculation incl. Bootstrapping " << PCorResultDenom[0]->GetLevel() << std::endl;
            ...
            for(usint i = 0; i < PCorResultDenom.size(); i++)
            {
                std::vector<Ciphertext<DCRTPoly>> partialCiphertextVecDenom, partialCiphertextVecNum;
                //distributed decryption
                //decryption of ciphertext before and after Bootstrapping to analyse precision
                for (usint j=0; j<numParties; j++){
                    if(j > 0){    // Backup: < numParties - 1            
                        partialCiphertextVecDenom.push_back(cryptoContext->MultipartyDecryptMain({PCorResultDenom[i]}, parties[j].kpShard.secretKey)[0]);
                        partialCiphertextVecNum.push_back(cryptoContext->MultipartyDecryptMain({PCorResultNum[i]}, parties[j].kpShard.secretKey)[0]);
                    }
                    else
                    {
                        partialCiphertextVecDenom.push_back(cryptoContext->MultipartyDecryptLead({PCorResultDenom[i]}, parties[j].kpShard.secretKey)[0]);
                        partialCiphertextVecNum.push_back(cryptoContext->MultipartyDecryptLead({PCorResultNum[i]}, parties[j].kpShard.secretKey)[0]);
                    }
                }

                Plaintext ptresultBSNum, ptresultBSDenom;

                cryptoContext->MultipartyDecryptFusion(partialCiphertextVecNum, &ptresultBSNum);
                ptresultBSNum->SetLength(numSlots);
                cryptoContext->MultipartyDecryptFusion(partialCiphertextVecDenom, &ptresultBSDenom);
                ptresultBSDenom->SetLength(numSlots);

                //Plaintext calculations
                double num      = ptresultBSNum->GetRealPackedValue()[0];    //All elements of plaintext are equal therefore its sufficent to read first element
                double denom    = ptresultBSDenom->GetRealPackedValue()[0];
        ...
        void CalcCipherVar(){
            ...
            for(usint i = 0; i < PCorResultDenom.size(); i++)
            {
                ...
                results.push_back(ptresultWoBS->GetRealPackedValue()[0] / numSlots);     //All elements of plaintext are equal therefore its sufficent to read first element
                resultsbs.push_back(ptresultBS->GetRealPackedValue()[0] / numSlots);
            }
            bm_results.DurDecryption += TOC_US(t_decr) / 1000.0;
        ...
        void CalcCipherSDev(){
            ...
            for(usint i = 0; i < SDevResult.size(); i++)
            {
                ...
                for (usint j=0; j<numParties; j++){
                    if(j < numParties - 1){                
                        partialCiphertextVec.push_back(cryptoContext->MultipartyDecryptMain({SDevResult[i]}, parties[j].kpShard.secretKey)[0]);
                    }
                    else
                    {
                        partialCiphertextVec.push_back(cryptoContext->MultipartyDecryptLead({SDevResult[i]}, parties[j].kpShard.secretKey)[0]);
                    }
                }
                ...
                resultsbs.push_back(sqrt((ptresultBS->GetRealPackedValue()[0] / numSlots)));
            }
            bm_results.DurDecryption += TOC_US(t_decr) / 1000.0;
...
int main(int argc, char* argv[]) {
    ...
    for (ScalingTechnique st :{ScalingTechnique::FIXEDAUTO, ScalingTechnique::FLEXIBLEAUTO, ScalingTechnique::FLEXIBLEAUTOEXT}) { // Rescale technique ScalingTechnique::FIXEDAUTO, ScalingTechnique::FLEXIBLEAUTO,
            ...
            for (int p : {3,6}){    // No of parties
                for (auto sf : {std::make_pair(40, 45), std::make_pair(50, 55), std::make_pair(59, 60)}) { //ScaleModSize and FirstModSize: 
                    for (COMPRESSION_LEVEL cl : {COMPRESSION_LEVEL::COMPACT, COMPRESSION_LEVEL::SLACK}) { // compression level
                        ...
                        std::cout << "##############################" << std::endl;
                        Benchmark(Filename, Repetitions, st, sl, p, sf.first, sf.second, cl, 5);
