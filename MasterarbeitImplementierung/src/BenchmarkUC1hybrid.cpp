...
struct Results {
    ...
    double PCor_Calc_DurCalcSqNum= 0.0;     //Duration of square calculation in numerator in Pearson Correlation calculation
    double PCor_Calc_DurCalcSq   = 0.0;     //Duration of square calculation in denominator in Pearson Correlation calculation
    double PCor_Calc_DurCalcDiv  = 0.0;     //Duration of division in Pearson Correlation calculation
    double PCor_Calc_DurCalcRSq  = 0.0;     //Duration of square in Pearson Correlation calculation    
    double PCor_Calc_Dur         = -1.0;    //Duration of calculating Pearson Correlation of all cipher text
    double PCor_BS_PrecAfter     = -1.0;    //precision of Pearson Correlation result in bit after Bootstrapping
    double PCor_Calc_PrecAfter   = -1.0;    //precision of Pearson Correlation result in bit before Bootstrapping
...
class FHEBenchmark {
    private:
        ...
        std::vector<Ciphertext<DCRTPoly>> PCorResultNum, PCorResultDenom;
    ...
    public:
        FHEBenchmark(int rescaletech, int batchsize, int scalemodsize, int firstmodsize, int ringdim, int deslvl, usint lvlbudget){
            ...
            depth = levelsAvailableAfterBootstrap + FHECKKSRNS::GetBootstrapDepth(levelBudget, secretKeyDist) + (numIterations - 1) -23;
        ...
        void Decrypt(){
            ...
            try{
                ...
                for(int i = 0; i < n; i++){
                    ...
                    Plaintext resultPCorNum, resultPCorDenom;
                    cryptoContext->Decrypt(keyPair.secretKey, PCorResultNum[i], &resultPCorNum);
                    resultPCorNum->SetLength(numSlots);
                    cryptoContext->Decrypt(keyPair.secretKey, PCorResultDenom[i], &resultPCorDenom);
                    resultPCorDenom->SetLength(numSlots);
        ...
        */
        ...
        void CalcCipherPearsonCorr(){
            ...
            std::vector<Ciphertext<DCRTPoly>> pcorresultsNum, pcorresultsDenom;
            ...
            for (const auto& [truckId, data] : CipherInput) {
                ...
                // numerator: (sum((x_i - x_avg)*(y_i - y_avg)))²  Berechnug Zähhler
                ...
                pcorresultsNum.push_back(numerator);
                ...
                pcorresultsDenom.push_back(denom3);
                // TIC(trsq);
                ...
                // TIC(tdiv);
                // auto denominator = cryptoContext->EvalDivide(denom3, lowerBound, upperBound, polydeg);
            ...
            bm_results.PCor_Calc_LvlAfter = pcorresultsDenom[0]->GetLevel();

            std::cout << "Level after Pearson Correlation: " << pcorresultsDenom[0]->GetLevel();
            ...
            for(size_t i =0; i < pcorresultsNum.size(); i++){
                std::stringstream output; // stream for output of openFHE lib
                std::streambuf* old_cout = std::cout.rdbuf(output.rdbuf()); //redirection of std::cout

                PCorResultNum.push_back(cryptoContext->EvalBootstrap(pcorresultsNum[i], 1U, 0U));
                std::cout.rdbuf(old_cout); // reset of std::cout
                std::vector<double> bstimings = stringToDoubleVector(output.str());
            ...
            for(size_t i =0; i < pcorresultsDenom.size(); i++){
                std::stringstream output; // stream for output of openFHE lib
                std::streambuf* old_cout = std::cout.rdbuf(output.rdbuf()); //redirection of std::cout

                PCorResultDenom.push_back(cryptoContext->EvalBootstrap(pcorresultsDenom[i], 1U, 0U));
                std::cout.rdbuf(old_cout); // reset of std::cout
                std::vector<double> bstimings = stringToDoubleVector(output.str());
            ...
            bm_results.PCor_BS_LvlAfter = PCorResultDenom[0]->GetLevel();
            ...
            try
            {
                ...
                for(size_t i = 0; i < pcorresultsNum.size(); i++)
                {
                    Plaintext resultPCorNum, resultPCorDenom, resultPCorBSNum, resultPCorBSDenom;
                    cryptoContext->Decrypt(keyPair.secretKey, pcorresultsNum[i], &resultPCorNum);
                    resultPCorNum->SetLength(numSlots);
                    cryptoContext->Decrypt(keyPair.secretKey, pcorresultsDenom[i], &resultPCorDenom);
                    resultPCorDenom->SetLength(numSlots);

                    //Bootstrapped ciphertexts
                    cryptoContext->Decrypt(keyPair.secretKey, PCorResultNum[i], &resultPCorBSNum);
                    resultPCorBSNum->SetLength(numSlots);
                    cryptoContext->Decrypt(keyPair.secretKey, PCorResultDenom[i], &resultPCorBSDenom);
                    resultPCorBSDenom->SetLength(numSlots);

                    // plaintext calculations
                    double num      = resultPCorNum->GetRealPackedValue()[0];         //All elements of plaintext are equal therefore its sufficent to read first element
                    double denom    = resultPCorDenom->GetRealPackedValue()[0];
                    double result   = num / sqrt(denom);

                    double numBS    = resultPCorBSNum->GetRealPackedValue()[0];
                    double denomBS  = resultPCorBSDenom->GetRealPackedValue()[0];
                    double resultBS = numBS / sqrt(denomBS);

                    results.push_back(result);    
                    resultsbs.push_back(resultBS);
                }
        ...
        void CalcCipherVar(){
            ...
            for (const auto& [truckId, data] : CipherInput) {
                ...
                bm_results.Var_Calc_DurCalcMean += TOC_US(tmean);
                 ...
                 // Sum all squared differences. 1/n will be done on plaintext calculation
                 auto cresult = cryptoContext->EvalSum(csq, numSlots);
            ...
            try
            {
                ...
                for(size_t i = 0; i < varresults.size(); i++)
                {
                    Plaintext resultVar, resultVarBS;
                    cryptoContext->Decrypt(keyPair.secretKey, varresults[i], &resultVar);
                    resultVar->SetLength(numSlots);

                    cryptoContext->Decrypt(keyPair.secretKey, VarResult[i], &resultVarBS);
                    resultVarBS->SetLength(numSlots);

                    //All elements of plaintext are equal therefore its sufficent to read first element
                    results.push_back(resultVar->GetRealPackedValue()[0] / numSlots);     
                    resultsbs.push_back(resultVarBS->GetRealPackedValue()[0] / numSlots);
                }
        ...
        void CalcCipherSDev(){
            ...
            for (const auto& [truckId, data] : CipherInput) {
                ...
                auto csum = cryptoContext->EvalSum(data.fuelConsumption, numSlots);
                auto cmean = cryptoContext->EvalMult(0.03125, csum);   // 1/32 = 0,03125
                bm_results.SDev_Calc_DurCalcMean += TOC_US(tmean);
                 ...
                 // Sum all squared differences. 1/n and sqrt() is done on plaintext level
                 auto cresult = cryptoContext->EvalSum(csq, numSlots);
            ...
            try
            {
                ...
                for(size_t i = 0; i < sdevresults.size(); i++)
                {
                    ...
                    results.push_back(sqrt((resultSDev->GetRealPackedValue()[0]/numSlots)));     //All elements of plaintext are equal therefore its sufficent to read first element
                    resultsbs.push_back(sqrt((resultSDevBS->GetRealPackedValue()[0]/numSlots)));
                }
        ...
        */
        ...
        void WriteJSON(double OverallDuration, std::string filename){
            ...
            newResult["PCor_BS_DurCoeffsToSlot"]    = bm_results.PCor_BS_DurCoeffsToSlot;
            newResult["PCor_BS_DurRaiseMod"]        = bm_results.PCor_BS_DurRaiseMod;
            newResult["PCor_BS_DurSine"]            = bm_results.PCor_BS_DurSine;
            newResult["PCor_BS_DurSlotsToCoeff"]    = bm_results.PCor_BS_DurSlotsToCoeff;
            ...
            newResult["SDev_BS_DurCoeffsToSlot"]    = bm_results.SDev_BS_DurCoeffsToSlot;
            newResult["SDev_BS_DurRaiseMod"]        = bm_results.SDev_BS_DurRaiseMod;
            newResult["SDev_BS_DurSine"]            = bm_results.SDev_BS_DurSine;
            newResult["SDev_BS_DurSlotsToCoeff"]    = bm_results.SDev_BS_DurSlotsToCoeff;
...
int main(int argc, char* argv[]) {
 ...
 //   int i = 1;
    ...
    // for (int j = 1; j = 2; ++j) { // Rescale technique from 1 to 3
    ...
    //             for (int l : {32768}) { // ring dimensionr: 1024, 2048, 4096, 8192, 16384, 32768, 65536
    //                 for (int h : {22, 25}){
    //                     for (int o: {1,2}){
    //                         std::cout << "##############################" << std::endl;
    ...
    //     //}
    // }
    //Benchmark(Filename, Repetitions, 3, 32, 44, 50, 32768, 22, 1);
   // Benchmark(Filename, Repetitions, 2, 32, 44, 50, 32768, 22, 1);
   //Benchmark(Filename, Repetitions, 2, 32, 50, 51, 32768, 22, 1);
   Benchmark(Filename, Repetitions, 2, 32, 59, 60, 1024, 22, 1);
